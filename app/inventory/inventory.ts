...
import {BEGINNING, COLORS, NAMES, OPPORTUNITY_NAMES, NOW, POKEMONS} from "./values";
...
export class Inventory {
    ...
    reset() {
        ...
        for (let i = 0; i < this.size; i++) {
            this._all.push({
                opportunity: OPPORTUNITY_NAMES[randomInt(OPPORTUNITY_NAMES.length)],
                customer: OPPORTUNITY_NAMES[randomInt(OPPORTUNITY_NAMES.length)],
                status: NAMES[randomInt(NAMES.length)],
                assignedTo: NAMES[randomInt(NAMES.length)],
                creation: new Date(BEGINNING + randomInt(NOW - BEGINNING)),
                createdBy: NAMES[randomInt(NAMES.length)]
            });
        }
    }
